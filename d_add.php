...
must_login();
$uid = (int)$_SESSION['uid']; $errs = [];
$d = ['judul'=>'','tanggal'=>date('Y-m-d'),'budget'=>'','lama'=>1,'status'=>0];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($d['judul']==='')          $errs[] = 'Judul wajib diisi.';
    if (!$d['tanggal'])            $errs[] = 'Tanggal wajib diisi.';
    if ($d['lama'] < 1)            $errs[] = 'Lama minimal 1 hari.';
    if (!is_numeric($d['budget'])) $errs[] = 'Budget tidak valid.';

    $foto = null; $eu = null;
    if (!$errs) { $foto = up_foto($_FILES['foto'] ?? [], $eu); if ($eu) $errs[] = $eu; }

    if (!$errs) {
        $b = (float)$d['budget'];
        $s = $conn->prepare("INSERT INTO destinasi (user_id,foto,judul,tanggal,budget,lama,status) VALUES (?,?,?,?,?,?,?)");
        $s->bind_param('isssdii', $uid, $foto, $d['judul'], $d['tanggal'], $b, $d['lama'], $d['status']);
        if ($s->execute()) { flash('s','Destinasi ditambahkan!'); header('Location: dest.php'); exit; }
        $errs[] = 'Gagal: ' . $conn->error;
    }
}
$title = 'Tambah Destinasi'; include __DIR__ . '/head.php';
?>
<div class="max-w-2xl mx-auto px-4 py-8">
  <h1 class="text-2xl font-extrabold mb-1">Tambah Destinasi</h1>
  <p class="text-slate-500 text-sm mb-6">Isi detail tujuan liburanmu.</p>
  <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 sm:p-8">
    ...
    <form method="post" enctype="multipart/form-data" class="space-y-5">
      <div><label class="block text-sm font-semibold mb-1">Foto</label>
        <input type="file" name="foto" accept="image/*"
          class="w-full text-sm file:mr-4 file:px-4 file:py-2 file:rounded-xl file:border-0 file:bg-sky-50 file:text-sky-600 file:font-bold">
        <p class="text-xs text-slate-400 mt-1">Maks 2MB (JPG/PNG/WEBP/GIF)</p></div>
      <div><label class="block text-sm font-semibold mb-1">Judul Liburan</label>
        <input type="text" name="judul" value="<?= e($d['judul']) ?>" required placeholder="Trip Bromo"
      ...
      <div class="grid sm:grid-cols-2 gap-4">
        <div><label class="block text-sm font-semibold mb-1">Budget (Rp)</label>
